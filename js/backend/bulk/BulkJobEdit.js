...
export default class BulkJobEdit {
	...
	render() {
		...
		_log('this.records', this.records);
		...
		this.records.forEach(product => {
			...
			product.original = product.original?.content || {};

			if ( product.calldata?._type == 'original' ) {
				originals[product.id] = product;
				return;
			}
			//product.original = originals[product.id] || {}
			product.content = product.content || {}
			const isExcluded = !this.ids.includes(product.id);
			const checkedAttr = isExcluded ? '' : 'checked';
			let modelName = product.content?.model?.replace(/-/g, ' ') || ''
			let clss = ''
			let status = '';
			let final = [];
			if ( product.content?.errors?.length ) {
				//final = product.content.errors.filter(e => e.final);
				//if (final.length) {
					clss = ' aai-error-product';
					status += '<span class="aai-error-box">error</span>'
				//}
			}
			if (product.calldata?._type == 'published' )
				status += `<span class="aai-success-box">${__('published', 'copyspell-ai')}</span>`;
			else if (product.calldata?._type == 'original' )
				status += `<span class="aai-info-box">${product.calldata?._type||''}</span>`;
			else
				status += `<span class="aai-gray-box">${product.calldata?._type||''}</span>`;
			...
			html += `
				<div class="aai-product-item${clss}" data-id="${product.id}">
					...
					<div class="aai-product-title">
						...
						${ product.content?.errors?.length ? `<div class="aai-product-error-summary">${product.content.errors.map(e => `<div class="aai-error-message">${e.error}</div>`).join('')}</div>` : '' }
					</div>
					<div class="aai-product-content">
						${final.length ?
					`<div class="aai-product-error">${product.content.errors.map(e => !e.final ? e.error : '').join('<br>')}</div>`
					:
					`${product.content.excerpt || product.original.excerpt || ''}`
				}
					</div>
	...
	previewProduct(product) {
		...
		$modal.innerHTML = `
			...
			<div class="aai-modal-footer">
				${!isSelected ? `<button class="aai-btn aai-btn-secondary aai-modal-mark">${__('Mark as Selected', 'copyspell-ai')}</button>` : ''}
				<button class="aai-btn aai-btn-primary aai-modal-update" data-id="${product.id}">${__('Publish Product', 'copyspell-ai')}</button>
			</div>
		`;
		...
		$overlay.querySelector('.aai-modal-mark')?.addEventListener('click', (e) => {
	...
	async UpdateSelected() {
		...
		try {
			...
			alert(`${__('Successfully published', 'copyspell-ai')} ${publishedCount} ${__('products', 'copyspell-ai')}.`);
			...
			_log('❌ error', error);
			this.hideLoader();
			alert(__('An error occurred while updating products.', 'copyspell-ai'));
		}
	...
	async updateProduct(e) {
		...
		$container.innerHTML = `
			<div class="aai-loader-container">
				${SVG.loader7}
				<div>${__('Publishing...', 'copyspell-ai')}</div>
			</div>
		`
	...
	statusUI() {
		let html = `
			...
			<button class="aai-btn aai-btn-primary aai-button-generate">${__('Publish Selected Products', 'copyspell-ai')}</button>
		`
